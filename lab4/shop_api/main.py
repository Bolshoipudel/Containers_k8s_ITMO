...
DATABASE_URL = f"postgresql+asyncpg://{os.getenv('POSTGRES_USER')}:{os.getenv('POSTGRES_PASSWORD')}@postgres-service:5432/{os.getenv('POSTGRES_DB')}"
...
async def patch_item(
    item_id: int, item: ItemPatch, session: AsyncSession = Depends(get_session)
):
    result = await session.execute(select(ItemDB).where(ItemDB.id == item_id))
    existing_item = result.scalar_one_or_none()
...
async def get_cart(cart_id: int, session: AsyncSession = Depends(get_session)):
    ...
    query = (
        ...
        .join(ItemDB, CartItemDB.item_id == ItemDB.id)
...
async def add_item_to_cart(
    cart_id: int, item_id: int, session: AsyncSession = Depends(get_session)
):
    result = await session.execute(select(CartDB).where(CartDB.id == cart_id))
    ...
    result = await session.execute(
        select(CartItemDB).where(
            CartItemDB.cart_id == cart_id, CartItemDB.item_id == item_id
        )
    )
...
async def chat_endpoint(websocket: WebSocket, chat_name: str):  # pragma: no cover
    ...
    try:
        ...
        chat_rooms[chat_name] = [
            (ws, user) for ws, user in chat_rooms[chat_name] if ws != websocket
        ]
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=int(os.getenv("SHOP_PORT", 8001)))