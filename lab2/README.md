...
Да, ограничить CPU и память на уровне контейнера, возможно
...
Однако стоит упомянуть, что спецификация :deploy предназначена для режима swarm. И при запуске `docker compose up something` не сработает.
